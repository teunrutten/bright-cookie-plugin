function AddCookies() {
  ...
  Array.from(buttons).forEach(function(button) {
    button.addEventListener('click', function(e) {
      ...
      if (popup) {
        var analytics_checkbox = popup.querySelector(
          '.js-cookie-checkbox-analytics'
        )
        var tracking_checkbox = popup.querySelector(
          '.js-cookie-checkbox-tracking'
        )
        var analytics_checkmark = popup.querySelector(
          '.js-cookie-checkmark-analytics'
        )
        var tracking_checkmark = popup.querySelector(
          '.js-cookie-checkmark-tracking'
        )

        // Analytics is ON if there's a checkmark, or if the checkbox is checked.
        if (
          analytics_checkmark ||
          (analytics_checkbox && analytics_checkbox.checked)
        ) {
          analytics = true
        }
        ...
        if (
          tracking_checkmark ||
          (tracking_checkbox && tracking_checkbox.checked)
        ) {
          tracking = true
        }
